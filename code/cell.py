"""Ported from godot/core.py, only the class Cell
"""
...
class Cell(object):
    ...
    def __repr__(self):
        return f'''{self.__class__}.{self.__class__.__name__} 
            exposure={self.exp:.3e},
            {len(self.we)} weights, mean {self.we.mean():.2f}
            times {self.tstart:.1f} to {self.tstop:.1f}'''
