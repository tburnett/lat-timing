...
class TimedData(object):
    ...
    def __init__(self, name, skycoord=None,  radius=5, 
        ...
        skycoord : a SkyCoord object [optional, could use name to look up] 
        ...
        print(f'Selected position: (l,b)=({l:.1f},{b:.1f})')
        ...
        print(f'Opening {len(files)} files, with {gbtotal:.1f} GB total')

        dflist=[]

        for filename in files:
            dflist.append(self.timeinfo(filename,l,b, radius=radius))
            print('.', end=' ')
        self.df = pd.concat(dflist)
        print(f'Selected {len(self.df)} photons within {radius} deg of source')

    def timeinfo(self, filename, l,b, radius, nside=1024):
