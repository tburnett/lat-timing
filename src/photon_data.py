...
class DataFiles(DocPublisher):
    """
    title: Input Files for Light Curve Processing 

    sections: basic_files source_example result
    ...
    def basic_files(self):
        ...
        * `ft2`: Livetime from {ft2_size} yearly FT2-format files. The fields used are 
        ...
        * `sources`: Files specific to the data in a cone about a source position are here, described 
        ...
        du_command = f'(cd {self.root}; du -h -d 1 . | column  -t)'
    ...
    def result(self):
        ...
        td_str = self.monospace(td)

        photon_data= td.photon_data
        
        lc = LightCurve(td.binned_weights())
    ...
    def source_example(self):
        ...
        its energy and band. For processing, this was copied from SLAC to the folder `/tmp/weight_files`.
        For Geminga we have
        {weight_file}

        Then we use main.Main, generating the following output:
        ...
        with self.capture_print() as text:
            self.tdata = Main( name=self.source_name, 
