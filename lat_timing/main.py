...
class Main(object):
    ...
    defaults=(
        ...
        ('position', None, 'postion,a tuple if name not recognized'),
    ...
    def __init__(self, **kwargs  ):
        ...
        assert self.name, 'Must specify a source name'
        self._set_geometry(self.name, self.position)

        self.timedata = self._load_data()
        if not self.timedata:
            raise Exception(f'could not load data') 

        if self.weight_file is not None:
            # adds weights from map and replace data object
            if os.path.isdir(self.weight_file):
                self.weight_file = os.path.join(self.weight_file, 
                                            self.name.replace(' ','_').replace('+','p')+'_weights.pkl')
